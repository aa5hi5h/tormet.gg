...
const  Dota2Page = () => {
  ...
  const [openMatches, setOpenMatches] = useState<Dota2MatchProps[]>([])
  const [playingMatches, setPlayingMatches] = useState<Dota2MatchProps[]>([])
  const [finishedMatches, setFinishedMatches] = useState<Dota2MatchProps[]>([])
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      for (const matchId of myActiveMatches) {
        const currentMatch = await getDota2MatchById(matchId)
  ...
  const loadAllMatches = async () => {
    try {
      const allMatches = await getAllDota2Matches() as any[]
  ...
  const handleCreateMatch = async () => {
    if (!username || !accountId) {
      setError('Please enter username and Dota 2 Account ID')
      return
    }
    ...
    try {
      const match = await CreateDota2Match(username, accountId, playerName || undefined)
  ...
  const handleJoinMatch = async (match: Dota2MatchProps) => {
    if (!username) {
    ...
    const joinerAccountId = prompt(`Enter your Dota 2 Account ID:`)
    if (!joinerAccountId) return
    ...
    try {
      await joinDota2Match(match.id, username, joinerAccountId, joinerName || undefined)
      await loadAllMatches()
  ...
  const getRandomImage = (matchId: string) => {
    ...
    return Dota2Images[hash % Dota2Images.length]
  }
  ...
  const getStatusBadge = (match: Dota2MatchProps) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-900">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-red-600/10 via-orange-600/10 to-gray-900/10">
              ...
              <div className="flex group">
                ...
                <div>
                  <div className="flex items-center gap-2 ml-4 mb-4">
                    ...
                    <span className="text-red-400 text-sm font-semibold">MOBA Strategy</span>
                  </div>
                  <h1 className="text-4xl font-black text-white leading-tight ml-4 mb-4">
                    DEFEND THE<br />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 via-orange-400 to-gray-400">
                      ANCIENT
                    </span>
                  </h1>
                  <p className="text-red-200 text-lg ml-4 mb-6">
                    Wager SOL and dominate the battlefield
                  </p>
                </div>
              </div>
            ...
            <div className="relative h-[500px] lg:h-[600px] z-10">
              ...
              <div className="absolute top-20 left-0 bg-black/80 backdrop-blur-md border border-red-500/30 px-6 py-4 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-3xl font-black text-white">#{username ? '1,245' : '---'}</div>
      ...
      <div className="container mx-auto px-6 py-12">
        <div className="flex flex-col mb-8">
          ...
          <p className="pl-1 text-slate-300">Battle for the Ancient and win prizes</p>
        </div>
                    ...
                    <div className="absolute bottom-3 left-3 right-3">
                      <div className="bg-black/60 backdrop-blur-sm rounded-lg p-2">
                        <p className="text-white font-bold text-sm truncate">
                          {match.summonerName1 || 'Dota 2 Player'}
                        </p>
                        <p className="text-red-300 text-xs">ID: {match.summonerPuuid1}</p>
                      </div>
                    </div>
                  </div>
                ...
                return (
                  <div
                    ...
                    className={`bg-zinc-900/90 backdrop-blur-sm border rounded-2xl p-6 ${
                      isParticipant ? 'border-red-500/30' : 'border-zinc-800'
                    }`}
                  >
                    ...
                    {isParticipant && (
                      <div className="bg-red-900/20 border border-red-500/30 rounded-xl p-5 mt-4">
                        <p className="font-bold mb-4 text-red-300 text-lg flex items-center gap-2">
                          <Zap className="w-5 h-5" />
                          Match Instructions
                        </p>
                        <ol className="space-y-2 text-sm list-decimal list-inside text-gray-300">
                          <li>Open Dota 2 client</li>
                          <li>Add opponent as friend (Steam ID: <strong className="text-orange-300">{match.creator.username === username ? match.summonerPuuid2 : match.summonerPuuid1}</strong>)</li>
                          <li>Create party OR play any game mode</li>
                          <li><strong className="text-red-300">IMPORTANT: Must be on OPPOSITE teams!</strong></li>
                          <li>Play the match (Radiant vs Dire)</li>
                          <li>Winner detected automatically via OpenDota! ⏱️ Checking every 10s...</li>
                        </ol>

                        <div className="mt-4 bg-orange-900/30 border border-orange-500/30 rounded-lg p-3">
                          <p className="text-xs text-orange-200">
                            💡 <strong>Tip:</strong> Easiest way is to create a lobby game (1v1 mid or full 5v5 with bots). Make sure "Expose Public Match Data" is enabled in Dota 2 settings!
                          </p>
                        </div>
                      </div>
                    )}
            ...
            <div className="space-y-4 text-sm text-gray-300">
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                ...
                <ol className="space-y-2 list-decimal list-inside">
                  <li>Create a match with your <strong>Dota 2 Account ID</strong> and wager amount</li>
                  <li>Wait for an opponent to join with their Account ID</li>
                  <li>Both players add each other as friends in Dota 2</li>
                  <li>Play any game mode (All Pick, Ranked, Turbo, Lobby) on opposite teams</li>
                  <li>Winner is automatically detected via OpenDota API!</li>
                </ol>
              </div>
              ...
              <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg p-4">
                ...
                <ol className="space-y-1 list-decimal list-inside text-xs">
                  <li>Open Dota 2 → Settings → Options</li>
                  <li>Enable "Expose Public Match Data"</li>
                  <li>Your Account ID is in your profile URL: <code className="bg-black/30 px-1 rounded">steamcommunity.com/profiles/[YOUR_ID]</code></li>
                  <li>Or use: <a href="https://www.dotabuff.com/" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Dotabuff.com</a> to find it</li>
                </ol>
              </div>
              ...
              <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4">
                ...
                <ul className="space-y-1 list-disc list-inside">
                  <li>You must be on <strong>opposite teams</strong> (Radiant vs Dire)</li>
                  <li>Winner detected automatically from match history</li>
                  <li>Same team = Match marked as draw</li>
                  <li>Public match data must be enabled</li>
                </ul>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs text-red-300 font-semibold mb-2 block">
                  DOTA 2 ACCOUNT ID <span className="text-orange-400">(numeric)</span>
                </label>
                <input
                ...
                />
                <p className="text-xs text-gray-400 mt-1">Find it in your Steam profile URL or Dotabuff</p>
              </div>
              ...
              <button 
                ...
                disabled={!username || !accountId || loading || parseFloat(wager || '0') <= 0}
                className="bg-gradient-to-r from-red-600 via-orange-600 to-gray-700 hover:from-red-500 hover:via-orange-500 hover:to-gray-600 text-white px-8 py-4 rounded-lg font-black text-lg w-full disabled:from-zinc-700 disabled:to-zinc-700 disabled:cursor-not-allowed transition-all shadow-lg shadow-red-500/20 hover:shadow-red-500/40"
              >
